...
The following trading strategy is an extension of my previous project (in collaboration with Olivier Haley) regarding the use of batteries and their profitability on the Australian power grid. The purpose is to take our initial findings and take a different approach while applying structured quantitative processes to improve and enhance the results. I have opted to look at a modeling structure that has the ability to capture the power of volatility that occurs in pricing in order to capitalize on price movements at the optimal time. Our previous apporach relied solely on time analysis. I now want to look at how I can use volatility analysis to potentially increase the profitability of the algorithm. For a detailed explanation of the Australian power grid, please refer to the [AEMC](https://www.aemc.gov.au/energy-system/electricity/electricity-system/NEM). This project will not go into as much depth explaining the grid system and pricing dynamics as our previous work. For access to that information, please visit [github](https://github.com/tigerwoodsjr/cetf).
...
Here again you can clearly see that there is seasonality in volatility. The winter season is highly volatile in comparison to the summer, but the idea that volatility increases during peak demand hours is very much still visable. Just looking at the fit of a regression on average price changes broken down by hour, day, and month, you'd expect to see a some significance, and it is relatively significant with an Adjusted R Squared of 0.51. All the months are statistically significant, and the high vol times of day are all statistically significant. 

```{r}
reg <- model_df %>% group_by(month, times) %>%
  summarise(change = ACTUAL_PRICE - lag(ACTUAL_PRICE), 
          mean_change = mean(change, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(month_name = factor(month, levels = 1:12, labels = month.abb))

slr <- lm(mean_change ~ factor(month) + factor(times), data = reg)

# summary(slr)
```


There is also both weekly and daily seasonality present, which is highlighted in the chart below, generated from Facebooks Prophet Model:
...
From a high level, these trends make sense if you think about them from the perspective of demand. The issue now becomes how to keep this seasonality in mind when producing trading signals. There will need to be variability in signal generation on a seasonal level. The reason for performing this analysis was to drive home that point heading into the next step, which is figuring out how to use volatility to trade on the grid and optimize the usage of the battery. 
...
#################################failed markov code (various levels of feature engineering were attempted) ###################################################
...
#test <- model_df %>% filter(DATETIME > as.POSIXct("2021-10-31 00:00:00"), DATETIME < as.POSIXct("2021-12-31 08:55:00"))

#  test %>% plot_ly(x = ~DATETIME,
#          y = ~ACTUAL_PRICE,
#          type = 'scatter',
#          mode = 'lines',
#          name = 'actual') %>% add_trace(data = test, x = ~DATETIME, y = ~RRP, type = 'scatter', mode = 'lines', name = 'fcast')
  

#fit <- lm(ACTUAL_PRICE ~ RRP + OPERATIONAL_DEMAND_POE10 + OPERATIONAL_DEMAND_POE50 + OPERATIONAL_DEMAND_POE90, data = model_df)
...
The model was trained on a set of arbirtrary parameters:

- SD Window of 2 hours.
- Buy percentile of 25%.
- Sell percentile of 75%. 

My expectation on the results of the training data will be that the volatility model will be profitable in the high volatility seasons, and the opposite on the others. Here is a plot of the cumulative equity on the training data with the arbitrary parameters, prior to optimization: 
...
check_total <- bind_rows(output_winter, output_spring, output_summer, output_fall) %>% arrange(DATETIME)

check_total %>% plot_ly(x = ~DATETIME, y = ~cumeq, type = 'scatter', mode = 'lines') %>% layout(title = 'Cumulative Profit Training Data', xaxis = list(title = ''))

```
A few notes: 

- The model performs better in the fall and winter (as expected).
- The warmer months kill performance.
- The model capitalizes on a large price swing, as well as loses on a large price swing (most likely outages).

Heading into optimization, I expect to see essentially the same result, which will answer the questions I laid out above as to why this model is not going to perform well in a market like this. The only change I'd expect to see is that optimization will smooth out the line, with less significant swings in profit. Ideally, the optimized fall and winter seasons should drive my profit up enough to remain positive throughout the year, and the optimizations during the spring and summer will mitigate the losses to a lesser amount. 

In an ideal world, as mentioned above, this is where my model would detect a regime switch and pivot strategies to something more ideal for that market state, such as a trend following model or a forcasting model relying on other parameters. 
...
My logic also appears to have a flaw. The sell percentile signal, over the course of optimization, does not want to change. After hours of debugging and redesigning the logic, the same problem persisted. I know this can't possibly be attributable to market conditions, and is a fault in my programming. I just let the model pinpoint one SD sell signal, and looked at the variations in the other two variables (hence the surface diagrams).

The predispatch and forcasted demand data all produced insignificant results on both prices and returns. As seen in our previous project, the highest significance we can find so far is time of day, broken down by day of week and month. 
