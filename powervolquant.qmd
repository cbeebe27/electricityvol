...
# Define your battery strategy function
battery_strategy <- function(df, sd_window, sd_buy_percentile, sd_sell_percentile) {
  ...
  df <- df %>%
    ...
    mutate(rollSd = na.approx(rollSd, na.rm = FALSE)) # Approximate missing rollSd values after aligning right
  
  # Initialize the state of charge (SOC)
  SOC <- startSOC
  cycle_count <- 0 # To track the number of cycles per day
  
  # Iterate over the rows of the dataframe to apply the battery constraints
  for (i in 1:nrow(df)) {
    
    if (df$signal[i] == 1 && SOC + ramp_rate_MWH <= maxSOC && cycle_count < max_cycles) {
      # Charge the battery
      SOC <- SOC + ramp_rate_MWH
      cycle_count <- cycle_count + cycles_per
    } else if (df$signal[i] == -1 && SOC - ramp_rate_MWH >= minSOC) {
      # Discharge the battery
      SOC <- SOC - ramp_rate_MWH
      cycle_count <- cycle_count + cycles_per
    } # No need for an else, as no action is taken when the signal is 0 or the battery is full/empty
    
    # Ensure SOC stays within bounds
    SOC <- pmin(pmax(SOC, minSOC), maxSOC)
    
    # Save the SOC and cycle count to the dataframe
    df$SOC[i] <- SOC
...
# Sample data to test the function (replace this with your actual dataframe)
# Let's assume `df` has a 'change' column for price changes
# df <- data_frame(DATETIME = seq(as.POSIXct("2021-01-01"), by = "5 min", length.out = 100), change = rnorm(100))

# Define the rolling standard deviation window and percentile thresholds
sd_window <- 24
sd_sell_percentile <- 0.75
sd_buy_percentile <- 0.25

# Call the strategy function
final_data <- battery_strategy(winter, sd_window, sd_buy_percentile, sd_sell_percentile)
