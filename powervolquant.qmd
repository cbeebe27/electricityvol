...
```{r}
pdf <- model_df %>% filter(year == 2022 & month %in% c(1, 4, 7, 10) & day == 20) %>% select(times, ACTUAL_PRICE, month) %>% 
...
window_size <- 24
dat <- model_df %>% select(DATETIME, ACTUAL_PRICE, times, year, month, week, weekdate, day) %>% 
...
modeldat <- model_df %>% mutate(Date = as.Date(DATETIME)) %>% 
                             select(Date, DATETIME, ACTUAL_PRICE, times, year, month, week, weekdate, day)
...
For now, I decided to pivot to looking at how I could use a GARCH model to provide insight on ways to use volatility to generate trading signals. 
```{r}
# chart vol through time. 
